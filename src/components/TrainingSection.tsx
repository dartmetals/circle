...
/* ── Person on bean bag illustration ─────────────────────── */
...
const SyllabusSection = () => {
  ...
  // Scroll animation for content - triggers based on hero section scroll
  useEffect(() => {
    const handleScroll = () => {
      if (sectionRef.current) {
        ...
        const heroScrollProgress = Math.min(1, Math.max(0, scrollY / heroHeight));
        
        // Content becomes visible when hero section is scrolled to 30%
        if (heroScrollProgress > 0.3) {
          // Progress from 0.3 to 1.0 maps to opacity 0 to 1
          const progress = Math.min(1, (heroScrollProgress - 0.3) / 0.7);
          setContentOpacity(progress);
          setContentTranslate(200 - (progress * 200));
        } else {
          setContentOpacity(0);
      ...
      >
        ...
        <div style={{ maxWidth: 1140, margin: '0 auto', display: 'grid',
          ...
          >
            ...
            {/* Items with dotted connector line */}
            <div style={{ position: 'relative' }}>
              ...
              <div style={{
                ...
                bottom: 36,
                width: 1,
                borderLeft: '1.5px dashed rgba(42,152,136,0.25)',
                zIndex: 0,
              }} />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 36, position: 'relative', zIndex: 1 }}>
                {syllabusItems.map((item, i) => (
                  <FadeUp key={item.num} delay={0.2 + i * 0.15} x={30}>
                    <div className="syllabus-item" style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
