...
const ClockIllustration = () => (
  <svg viewBox="0 0 140 120" width="140" height="120" fill="none">
    <circle cx="70" cy="68" r="44" stroke="#b0d0cc" strokeWidth="2" fill="rgba(168,210,205,0.1)" />
...
interface CourseCardProps { tag: string; title: string; illustration: React.ReactNode; delay: number; }

const courses = [
  { tag: 'DESIGN',      title: 'How to make a quality site in WordPress in 40 hours without experience', illustration: <ClockIllustration /> },
  { tag: 'PROGRAMMING', title: 'WordPress: How to start in a developer without any basic knowledge',    illustration: <ProgrammingIllustration /> },
  { tag: 'SEO',         title: 'Creating dynamic sites with CMS WordPress + SEO for WordPress websites', illustration: <SEOIllustration /> },
];
    ...
    >
      <motion.div
        ...
        style={{
          ...
          background: '#ffffff', borderRadius: '50% 50% 0 0',
          transformOrigin: 'bottom', zIndex: 0,
        }}
      />
      <div style={{ position: 'relative', zIndex: 1 }}>
        <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '0.16em', color: '#6a9898', textTransform: 'uppercase', fontFamily: "'DM Sans', sans-serif" }}>{tag}</span>
        <h3 style={{ fontSize: 13.5, fontWeight: 500, color: '#1a3535', lineHeight: 1.6, flex: 1, margin: 0, fontFamily: "'DM Sans', sans-serif" }}>{title}</h3>
        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end', paddingTop: 4 }}>{illustration}</div>
        <div style={{ borderTop: '1px solid rgba(0,0,0,0.07)', paddingTop: 12, display: 'flex', alignItems: 'center' }}>
          <motion.span style={{ fontSize: 11, fontWeight: 700, color: '#2a8a78', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', gap: 5, fontFamily: "'DM Sans', sans-serif", cursor: 'pointer' }} whileHover={{ gap: 10 }}>
            READ MORE →
          </motion.span>
        </div>
      </div>
...
const CoursesSection = () => (
  <>
    <style>{`
      ...
      /* ✅ Mobile/tablet: allow full height growth */
    ...
    <section
      className="courses-root courses-section-el"
      style={{
        background: 'linear-gradient(150deg, #1a7272 0%, #258f8f 32%, #c0a882 72%, #d8c09a 100%)',
        padding: '90px 56px',
        fontFamily: "'DM Sans', sans-serif",
        overflow: 'hidden',          /* ✅ clip to exact viewport height on desktop */
        position: 'relative',
        /* ✅ CHANGED: height: 100vh — exact screen height on every laptop/desktop size */
        height: '100vh',
    ...
    >
      <div style={{ maxWidth: 1140, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 48, gap: 40, flexWrap: 'wrap' }}>
          <FadeUp>
            <h2 style={{ fontFamily: "'DM Serif Display', Georgia, serif", fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 400, color: '#fff', lineHeight: 1.18, margin: 0, maxWidth: 420 }}>
              Professional courses<br />on creating websites
            </h2>
          </FadeUp>
          <FadeUp delay={0.18}>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.75, maxWidth: 290, margin: 0, paddingTop: 6 }}>
        ...
        <div className="courses-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          {courses.map((c, i) => <CourseCard key={c.tag} {...c} delay={0.1 + i * 0.15} />)}
        </div>
      </div>
    </section>
  </>
